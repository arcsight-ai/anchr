...
function walk(root: string, base: string, collected: string[]): void {
  ...
  for (const name of names) {
    ...
    if (isSymlink(absPath)) continue;

    if (entryIsExcludedDir(name)) continue;
...
function entryIsExcludedDir(name: string): boolean {
  return EXCLUDED_DIRS.has(name);
}

function entryIsDir(entry: { absPath: string }): boolean {
  try {
...
function entryIsSourceFile(entry: { name: string }): boolean {
  const ext = entry.name.slice(entry.name.lastIndexOf("."));
  return EXTENSIONS.has(ext);
}

function fileSizeOk(absPath: string): boolean {
  try {
    const stats = statSync(absPath, { throwIfNoEntry: false });
