...
import type { Edge, EdgeKind, ModuleID } from "./types.js";

function buildReexportAdjacency(edges: Edge[]): Map<ModuleID, ModuleID[]> {
...
const closureCache = new Map<ModuleID, ModuleID[]>();

function publicClosure(
  start: ModuleID,
...
export function propagatePublicSurface(edges: Edge[]): Edge[] {
  closureCache.clear();

  const adj = buildReexportAdjacency(edges);
  const result: Edge[] = [...edges];
  const added = new Set<string>();

  for (const e of edges) {
    if (e.kind !== "value-import") continue;
